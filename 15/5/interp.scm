...
;;;               (extend-env id (newref val) env))))
	  ...
	  (proc-val
        ...
        (newarray-exp (size initial-value)
                      (make-array (expval->num (value-of size env)) initial-value))
        
        (arrayref-exp (array-ref index)
                      (let ((pointer (value-of (var-exp array-ref) env)))
                        (let ((index-number (expval->num (value-of index env))))
                          (dereference-array-index 
                               pointer index-number))))
        
        (arrayset-exp (array-ref index val)
                      (let ((pointer (value-of (var-exp array-ref) env)))
                        (let ((index-number (expval->num (value-of index env))))
                          (set-array-index 
                             pointer
